...
def build_dataset(sample_count: int = 512, max_history_length: int = 8) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    generator = torch.Generator().manual_seed(7)
    targets = torch.randint(1, 101, (sample_count,), generator=generator)
    lengths = torch.randint(1, max_history_length + 1, (sample_count,), generator=generator)
    ...
    return histories, targets, labels


def main() -> None:
    torch.manual_seed(7)
    histories, targets, labels = build_dataset()
    model = BST(num_items=101)
