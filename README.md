...
面向工业推荐系统的中文知识整理项目。当前按“评测指标、推荐系统整体架构、冷启动问题”三个顶层问题域组织，其中推荐系统整体架构进一步拆分为 Index 池、L1 召回、L2 精排和 L3 后处理。
...
模型脚本是用于理解数据流、模型结构和损失函数的最小示例，不直接等同于生产实现。I2I、MMR 和 Top-K 指标仅依赖 Python 标准库；双塔、协同过滤、生成式召回、DeepFM 和 DIN 依赖 PyTorch。冷启动问题按业务问题、方法机制和案例组织，不单列某个模型作为完整解决方案。

## 维护约定

- 顶层 `README.md` 负责目录导航；各问题域的综述文档负责系统性论述；实现目录中的 `README.md` 负责单个模型。
