...
│   │   ├── 双塔召回/{数据构造,模型架构}/
│   │   ├── 协同过滤/
│   │   ├── I2I召回/{ItemCF,Item2Vec,InfoNCE,SimCSE}/
│   │   ├── 生成式召回/
│   │   └── 其他召回方式/{热门与趋势召回,内容检索召回,图与知识召回,规则与运营召回,探索召回}/
│   ├── L2精排/
│   │   ├── README.md
│   │   ├── 综述.md
│   │   └── 主要实现架构/
...
模型脚本是用于理解数据流、模型结构和损失函数的最小示例，不直接等同于生产实现。统计 ItemCF、邻域 CF、负采样、语义 ID Trie 和 MMR 仅依赖 Python 标准库；双塔、BPR、Item2Vec、对比学习、生成式 next-item、DeepFM 和 DIN 依赖 PyTorch。冷启动问题按业务问题、方法机制和案例组织，不单列某个模型作为完整解决方案。

## 维护约定

- 顶层 `README.md` 负责目录导航；各问题域的综述文档负责系统性论述；实现目录中的 `README.md` 负责单个模型；顶层文献目录负责论文索引。
- 单个方法解释其特有的数据、结构、损失和推理方式；ANN 与索引生命周期归属 Index 池，召回编排、融合、降级和监控归属召回系统综述。
